...
/**
 * Create Folder for Store CSS Files
*/
if ( !function_exists('eb_create_directory') ) {
    function eb_create_directory() {
    
        $upload = wp_upload_dir();
        $upload_dir = $upload['basedir'];
        $upload_dir = $upload_dir . '/eb-styles';
        if (! is_dir($upload_dir)) {
            mkdir( $upload_dir, 0700 );
        }
    }
    register_activation_hook( __FILE__, 'eb_create_directory' );
}